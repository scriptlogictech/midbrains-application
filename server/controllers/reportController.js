  ...
  try {
    ...
    const [
      ...
      req.user.role === "super_admin"
        ...
        : Company.countDocuments({ _id: req.user.company }),

      Lead.countDocuments(companyFilter),

      Admission.countDocuments(companyFilter),

      Internship.countDocuments(companyFilter),

      CorporateTraining.countDocuments(companyFilter),

      Project.countDocuments(companyFilter),

      Placement.countDocuments(companyFilter),
    ]);
...
exports.revenueReport = async (req, res) => {
  try {
    ...
    const [admissionRevenue, projectRevenue, corporateTrainingRevenue] =
      await Promise.all([
        Admission.aggregate([
          { $match: companyFilter },
          {
    ...
    const admissionTotal = admissionRevenue[0]?.total || 0;
    const projectTotal = projectRevenue[0]?.total || 0;
    const corporateTrainingTotal =
      corporateTrainingRevenue[0]?.total || 0;
    ...
    res.status(200).json({
      success: true,
      data: {
        admissionRevenue: admissionTotal,
        projectRevenue: projectTotal,
        corporateTrainingRevenue: corporateTrainingTotal,
        totalRevenue,
      },
    });
  } catch (error) {
...
// PLACEMENT REPORT
// ==========================================
exports.placementReport = async (req, res) => {
  try {
    const companyFilter = getCompanyFilter(req);
    ...
    const placementRate =
      total > 0 ? Number(((selected / total) * 100).toFixed(2)) : 0;
    ...
    res.status(500).json({
      success: false,
      message: "Failed to fetch placement report",
      error: error.message,
    });
  }
};